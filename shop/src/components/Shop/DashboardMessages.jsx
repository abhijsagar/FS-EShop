        ...
        <div
            ...
            <div className='pl-3'>
                ...
                <p className='text-[16px] text-[#000c]'>
                    {!isLoading && data?.lastMessageId !== user?._id ? 'You:' : user?.name?.split(' ')[0] + ': '} {data?.lastMessage}
                </p>
            </div>
        </div>
    );
                    ...
                    messages.map((item, index) => {
                        return (
                            <div className={`flex w-full my-2 ${item.sender === sellerId ? 'justify-end' : 'justify-start'}`} ref={scrollRef}>
                                ...
                                {item.images && <img src={`${item.images?.url}`} className='w-[300px] h-[300px] object-cover rounded-[10px] mr-2' />}
                                {item.text !== '' && (
                                    <div>
